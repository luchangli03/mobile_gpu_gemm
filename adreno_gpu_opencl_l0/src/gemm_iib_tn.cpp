...
__kernel void gemm_kernel(__read_only image2d_t img_a, __read_only image2d_t img_b, __global DTYPE *d_c, const int m,
                          const int n, const int k) {
  int gx = get_global_id(0); // [0, N / 4)
  int gy = get_global_id(1); // [0, M / 8)

  // do not use pack8
  DTYPE_PACK4 a0[KTILE];
  DTYPE_PACK4 a1[KTILE];
  DTYPE_PACK4 b[KTILE];
  DTYPE_PACK4 c[MTILE];
  ...
  for (int ktcnt = 0; ktcnt < kloop; ktcnt++) {
    ...
    // it seems we should read same image continiously, rather than Cross reading different images
#pragma unroll
...
#pragma unroll
    for (int i = 0; i < KTILE; i++) {
      a0[i] = READ_IMAGE_FUNC(img_a, (int2)(a_x_off, kpos + i));
      a1[i] = READ_IMAGE_FUNC(img_a, (int2)(a_x_off + 1, kpos + i));
    }

#pragma unroll
    for (int i = 0; i < KTILE; i++) {
      c[0] = mad(a0[i].s0, b[i], c[0]);
      c[1] = mad(a0[i].s1, b[i], c[1]);
      c[2] = mad(a0[i].s2, b[i], c[2]);
      c[3] = mad(a0[i].s3, b[i], c[3]);

      c[4] = mad(a1[i].s0, b[i], c[4]);
      c[5] = mad(a1[i].s1, b[i], c[5]);
      c[6] = mad(a1[i].s2, b[i], c[6]);
      c[7] = mad(a1[i].s3, b[i], c[7]);
    }
  }
